...
if($_POST) 
{
	...
	if (stristr($mff,"primary_key")) { // si 1er champ est une cl√© primaire
		// on regarde si c'est un auto incr√©ment
...
if($_GET['action'] == 'ajout') 
{
	...
	// on ajoute en hidden la categorie parent et l'entit√©e parent
...
// Edition ou Consultation d'une entit√©
// -------------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------------
...
//	$CIL=InitPOReq($sql,'annuaire_externe');
	...
	// rÈcupËre le nom (Èventuel) de la table virtuelle dÈcrivant les champs spÈciaux
